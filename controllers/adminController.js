    ...
    try {
        ...
        if (req.files) {
            if (req.files.profileImage) {
                profileImagePath = `${host}/uploads/${req.files.profileImage[0].filename}`;
            }
            if (req.files.favicon) {
                faviconPath = `${host}/uploads/${req.files.favicon[0].filename}`;
            }
            if (req.files.resume) {
                resumePath = `${host}/uploads/${req.files.resume[0].filename}`;
            }
        }
...
const updateCertification = async (req, res) => {
    try {
        ...
        if (req.file) {
            const host = process.env.BACKEND_URL || `${req.protocol}://${req.get('host')}`;
            cert.imageUrl = `${host}/uploads/${req.file.filename}`;
        }
